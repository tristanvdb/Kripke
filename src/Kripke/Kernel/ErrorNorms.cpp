		  ...
		  double                  *vol_grp) const
  {
    ...
    RAJA::kernel<ExecPolicy>(
			     ...
			     KRIPKE_LAMBDA (Direction d, Group g, ZoneK k, ZoneJ j, ZoneI i) {
			       ...
			       if ( false && *z == 0 )//|| psi_exact>1e-2)
				 {
				   std::cout<<"i, j, k : "<<*i<<", "<<*j<<", "<<*k<<std::endl;
				   std::cout<<view_id(d)*xcos(d)<<", "<<view_jd(d)*ycos(d)<<", "<<view_kd(d)*zcos(d)<<std::endl;
				   std::cout<<"zone : "<<*z<<" : x, y, z "<<xz<<", "<<yz<<", "<<zz<<" : psi = "<<psi(d,g,z)<<" : psi_e = "<<psi_exact<<" : err = "<<err<<std::endl;
				 }
