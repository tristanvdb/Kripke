    ...
    RAJA::kernel<ExecPolicy>(
			     ...
			     KRIPKE_LAMBDA (Direction d, Group g, ZoneK k, ZoneJ j, ZoneI i) {
			       ...
			       double psi_exact = compute_exact_solution<double>(xz,yz,zz,
										 ...
										 ksn_problem);
			       double err = std::abs(psi(d,g,i,j,k) - psi_exact);

						     
			       max_nrm_red.max(err);
			       l1_nrm_red += w(d) * volume(i,j,k) * err;
			       l2_nrm_red += w(d) * volume(i,j,k) * err * err;

			       max_sol_nrm_red.max(std::abs(psi(d,g,i,j,k)));
			       l1_sol_nrm_red += w(d) * volume(i,j,k) * std::abs(psi(d,g,i,j,k));
			       l2_sol_nrm_red += w(d) * volume(i,j,k) * std::abs(psi(d,g,i,j,k)) * std::abs(psi(d,g,i,j,k));

			       vol_grp_red += w(d) * volume(i,j,k);
			     }
			     );
...
void Kripke::Kernel::error_norms(Kripke::Core::DataStore &data_store)
{
  ...
  double vol_grp = 0.0;
  auto &field_psi = data_store.getVariable<Kripke::Field_Flux_psi>("psi");
